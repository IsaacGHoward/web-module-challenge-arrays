...
console.log("TASK 2 :  V V V V V V")
...
console.log("TASK 3 :  V V V V V V")
...
function removeFlavorByName(flavors, flavorName){
    if(flavors.includes(flavorName)){
        flavors.splice(flavors.indexOf(flavorName));
        return(flavors);
    }
    else{
        return("No flavor in array");
    }

}
console.log("TASK 5 :  V V V V V V");
...
function copy(flavorsCopy, originalFlavors){
    flavorsCopy = [...originalFlavors];
    return(flavorsCopy);
}
let flavorsCopy;
console.log("TASK 6 :  V V V V V V");
console.log(copy(flavorsCopy, originalFlavors));
...
function filterByWord(flavors, promo){
    let filterArray = [];
    for(let i=flavors.length-1; i>0; i--){
        if(flavors[i].includes(promo)){
            filterArray.unshift(flavors[i]);
            flavors.splice(i,1);
        }
    }
    return(filterArray);
...
function getAverageWordLength(theArray){
    let totalWords = 0;
    let arrayLength = theArray.length;
    for(let i=0;i<arrayLength;i++){
        var words = theArray[i].split(" ");
        totalWords += words.length;
    }
    return(totalWords / arrayLength);
}
var testWords = ["One Two Three", "One Two", "One", "One" , "One"] //8 / 5 should be 1.6
console.log("STRETCH : " + getAverageWordLength(testWords));
